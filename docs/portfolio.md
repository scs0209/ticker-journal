...
> 기능 구현이 끝날 때마다 이 문서와 아래 그림을 갱신한다.
...
노션·시트에 흩어진 주식 리서치·매매 이유를 **종목 타임라인**으로 묶고, **모바일에서 입력 · 웹에서 검색·정리**하며, **App Store / Play Store**까지 가는 크로스플랫폼 앱.
...
flowchart TB
  ...
  subgraph backend [Backend - Phase 0+]
...
gantt
  ...
  section Now
  Scaffold + tests           :done, s0, 2026-08-11, 1d
  section Phase 0
  Auth + CRUD mobile         :p0, 2026-08-12, 10d
  section Phase 1
...
| web | Vitest + RTL | UI 계약 |
| web | Playwright | 브라우저 플로우 |
| mobile | jest-expo + RNTL | 네이티브 화면 |
...
### 1.5 아키텍처 freeze (2026-08-13)

- Phase 0 착수 전 기준선: `docs/architecture.md` (모노레포 · Supabase/RLS · 도메인 · ADR)

### 1.6 이후 로드맵

- [ ] Phase 0: Supabase Auth + CRUD (앱)
- [ ] Phase 1: 웹 검색·종목 상세
...
| 테스트 | React 19.2.3 vs react-dom 19.2.8 → RTL 빈 DOM | `pnpm.overrides`로 정렬 |
...
- 모노레포 공유 스키마 = “풀스택 모바일” 한 줄.
- React 버전 불일치는 RTL이 **조용히 빈 트리**를 그린다.
- 다이어그램(와이어·머메이드)을 문서에 두면 면접·노션 포트폴리오에 바로 붙일 수 있다.
...
Ticker Journal은 주식 리서치 스크랩과 매매 이유를 종목 타임라인으로 관리하는 크로스플랫폼 앱입니다. 모바일 입력 + WebView 차트, 웹 검색·정리를 같은 계정으로 제공합니다.

### 내가 한 일

- 문제 정의·MVP 3화면 와이어프레임·아키텍처 다이어그램 작성
- pnpm/Turborepo 모노레포 (`mobile` / `web` / `shared`)
- Expo Router + WebView 뼈대, Zod 공유 스키마
- Vitest / RTL / Playwright / jest-expo 테스트 계층
- (예정) Supabase · 스토어 2곳 배포

### 성과 / 임팩트

- *(스토어 URL, 실사용 종목 N·주간 M — `docs/resume-bullets.md` 지표 표 참고)*
